...
| --- | --- |
| Bounds alignment | **World** matches the Model Builder template button. Choose **Model** to follow the current pivot axes when a rotated build gets oversized world bounds. |
| Pivot position | **Center** uses the bounds center. **Bottom** puts the pivot at the bottom center of the padded bounds along the chosen Y axis. **Keep pivot** preserves its current position and rotation. The helper keeps its full bounding-box size in every mode. |
| Padding per side | Add studs to every side when you want extra spacing between placed templates. |
| Ignore invisible parts | Exclude fully transparent hitboxes and helpers that would otherwise enlarge the bounds. Generated PrimaryParts are always excluded. |
| Existing PrimaryPart | **Skip** preserves existing assignments. **Replace** refits a helper generated by this tool, or assigns a new helper while keeping the original part. |
| Anchor generated part | Leave on for static templates. For moving assemblies, turn it off and weld the helper to the assembly yourself. |

The helper has collision, touch, and raycast interaction disabled. The tool does not create welds. It skips generated helpers connected to other parts so refitting cannot move their assemblies. Models with no included parts or bounds larger than 2048 studs are skipped with an explanation. Meshes and unions use their part bounds. Settings are saved, and each batch supports Undo.

The Model Builder template's **Add primary part** button uses the same generator with world alignment and a center pivot. Repeated clicks refit its generated helper without accumulating padding or duplicate parts.
